...
*   **详细阅卷记录保存**：所有阅卷结果将自动保存为Excel格式文件，按日期和阅卷模式（单评/双评）分类存储。记录内容包括：
...
*   **分数处理精度**：AI返回的原始分数会首先经过校验（确保在题目设定的最低分和最高分之间）；然后，此校验后的分数将被四舍五入到最接近的配置步长（支持 0.5 或 1.0）；最后，这个经过步长处理的分数会再次被校验以确保其仍在题目配置的最低分和最高分范围内，最终用于输入。
...
*   **csv**：基础Python库（用于数据处理，主要记录格式为Excel）。
...
所有阅卷结果将保存在程序根目录下的 `阅卷记录/` 文件夹中，格式为 **Excel (.xlsx)** 文件。

*   **目录结构**：`阅卷记录/YYYY年MM月DD日/`
    *   阅卷记录按日期创建子文件夹。
*   **文件命名**：
    *   单题模式：`此题最高{满分}分_{单评/双评}.xlsx`
    *   多题模式：`共阅{题数}题_{单评/双评}.xlsx`
*   **Excel 文件内容**：
    *   **记录类型**：包含详细记录（每道题的评分详情）和汇总记录（批次统计信息）
    *   **单评模式详细记录列**：
        *   `时间` - 阅卷时间戳
        *   `题目编号` - 题目序号
        *   `学生答案摘要` - AI提取的答案摘要
    ...
    *   **双评模式详细记录列**：
        ...
        *   `OCR识别原文` - OCR识别的完整文本内容（如启用OCR）
    *   **汇总记录**：
        *   批次完成时间、状态、完成题数、总用时、评分模式、使用的AI模型等信息
